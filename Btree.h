...
private:
    ...
    bool  Encontrar( B_node<Type, order> *current, Nino *&target);

public: // publics.
	Btree();
	// travesling
	void inOrder();
    bool buscar( Nino *&searchitem );
	void mostrarArbol();
	// insertion
    bool buscarEnNodo( B_node<Type, order> *current, Nino* &target, int &position );
    void insertar(Nino* &new_entry);
    void insertarEnRecursion( B_node<Type, order> *current, Nino* &new_entry,Nino* &median,B_node<Type, order> * &rightchilds , bool &result );
    void insertarEnNodo(B_node<Type, order> *current, Nino* &entry, B_node<Type, order> *rightchilds, int position);
    void dividirNodo( B_node<Type, order> *current,  Nino* &extra_entry,   B_node<Type, order> *extra_childs,  int position,
        B_node<Type, order> * &right_half,   Nino* &median);
	// eliminacion

    void remover( Nino *&target );
    void removerEnRecursion( B_node<Type, order> *current, Nino* &target );
...
template <class Type, int order> bool Btree<Type,order>::buscar(Nino *&searchitem ){
	
	return Encontrar( root , searchitem );
}
template <class Type, int order> bool Btree<Type,order>::Encontrar( B_node<Type, order> *current, Nino* &target ){
...
template <class Type, int order> bool Btree<Type,order>::buscarEnNodo(B_node<Type, order> *current, Nino *&target, int &position ){
	position=0;
...
template <class Type, int order> void Btree<Type,order>::insertar(Nino *&new_entry){
    Nino* median;
...
template <class Type, int order> void Btree<Type,order>::insertarEnRecursion(B_node<Type, order> *current,
    Nino *&new_entry, Nino *&median, B_node<Type, order> * &rightchilds , bool &result ){
...
template <class Type, int order> void Btree<Type,order>::insertarEnNodo(B_node<Type, order> *current,
    Nino *&entry, B_node<Type, order> *rightchilds, int position){
		int i; 
...
template <class Type, int order> void Btree<Type,order>::dividirNodo(B_node<Type, order> *current, Nino *&extra_entry,   B_node<Type, order> *extra_childs,  int position,
    B_node<Type, order> * &right_half,   Nino *&median){
...
template <class Type, int order> void Btree<Type,order>::remover(Nino *&target ){
...
template <class Type, int order> void Btree<Type,order>::removerEnRecursion(B_node<Type, order> *current, Nino *&target ){
